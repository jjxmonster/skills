...
| Skill | Description |
|-------|-------------|
| [`add-adr`](skills/add-adr/SKILL.md) | Turns a decision made in the conversation into a new Architecture Decision Record at `.ai/adrs/<id>/adr.md` — and if no decision qualifies, writes nothing and asks what you want to record instead. |
| [`blast-radius`](skills/blast-radius/SKILL.md) | Reports what a JavaScript/TypeScript change affects outside its own diff — broken consumers, changed contracts, and coupling that a symbol-name grep misses. |
